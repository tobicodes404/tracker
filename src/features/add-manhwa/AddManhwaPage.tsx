...
export function AddManhwaPage() {
  ...
  useEffect(() => {
    const loadSuggestions = async () => {
      ...
      if (formData.author.length >= 2) {
        const allManhwa = await db.manhwa.toArray();
        const matches = [...new Set(allManhwa
          .filter(m => m.author && m.author.toLowerCase().includes(formData.author.toLowerCase()))
          .map(m => m.author))].slice(0, 5);
        setAuthorSuggestions(matches);
      } else setAuthorSuggestions([]);

      if (formData.artist.length >= 2) {
        const allManhwa = await db.manhwa.toArray();
        const matches = [...new Set(allManhwa
          .filter(m => m.artist && m.artist.toLowerCase().includes(formData.artist.toLowerCase()))
          .map(m => m.artist))].slice(0, 5);
        setArtistSuggestions(matches);
      } else setArtistSuggestions([]);
    };
    loadSuggestions();
  }, [formData.title, formData.author, formData.artist]);
  ...
  const handleFemaleCharImage = (index: number, file: File | null) => {
    ...
    if (file) {
      setFemaleCharImages(prev => ({ ...prev, [`female-${index}`]: file }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const id = await createManualManhwa({
        ...formData,
      ...
      <form onSubmit={handleSubmit} className="p-4 space-y-5">
        ...
        <div className="flex flex-col items-center space-y-3">
          <div 
            ...
            className="w-32 h-48 bg-neutral-800 rounded-lg border-2 border-dashed border-neutral-600 flex items-center justify-center cursor-pointer overflow-hidden hover:border-blue-500 transition-colors"
          >
            {coverPreview ? (
            ...
            <div className="absolute z-20 w-full bg-neutral-800 border border-neutral-700 rounded-lg mt-1 max-h-40 overflow-y-auto shadow-lg">
              {titleSuggestions.map((title, i) => (
                <button
                  ...
                  onClick={() => { setFormData({...formData, title}); setShowTitleSuggestions(false); }}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <datalist id="author-list">
              {authorSuggestions.map((name, i) => <option key={i} value={name} />)}
            </datalist>
          </div>
          <div>
            ...
            <datalist id="artist-list">
              {artistSuggestions.map((name, i) => <option key={i} value={name} />)}
            </datalist>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between bg-red-900/20 border border-red-800 p-3 rounded-lg">
          ...
          <button
            ...
            className={`w-12 h-6 rounded-full transition-colors relative ${formData.isAdult ? 'bg-red-600' : 'bg-neutral-600'}`}
          >
        ...
        <div>
          ...
          <textarea 
            ...
            rows={3} 
            className="w-full bg-neutral-800 border border-neutral-700 rounded-lg px-3 py-2.5 text-white focus:outline-none focus:border-blue-500 resize-none"
          ...
          {[
            ...
            <div key={r.key}>
              ...
              <input
                ...
                min="0"
                max="10"
                value={ratings[r.key as keyof typeof ratings]}
                onChange={(e) => setRatings({...ratings, [r.key]: Number(e.target.value)})}
        ...
        <div className="bg-neutral-800 p-4 rounded-xl border border-neutral-700 space-y-3">
          <h3 className="font-semibold text-blue-400"> Male Characters</h3>
          <div className="flex gap-2">
            <input
              type="text"
              value={newMaleChar}
              onChange={(e) => setNewMaleChar(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), addMaleCharacter())}
              ...
              {maleCharacters.map((char, i) => (
                <div key={i} className="flex items-center gap-2 bg-neutral-900 p-2 rounded-lg">
                  <label className="w-10 h-10 bg-neutral-700 rounded-lg flex items-center justify-center cursor-pointer overflow-hidden flex-shrink-0">
                    {char.imageFile ? (
                      <img src={URL.createObjectURL(char.imageFile)} alt="" className="w-full h-full object-cover" />
                    ) : (
                    ...
                    <input 
                      ...
                      accept="image/*" 
                      className="hidden" 
                      onChange={(e) => handleMaleCharImage(i, e.target.files?.[0] || null)}
                    />
                  </label>
        ...
        {/* Female Characters */}
        <div className="bg-neutral-800 p-4 rounded-xl border border-neutral-700 space-y-3">
          <h3 className="font-semibold text-pink-400"> Female Characters</h3>
          <div className="flex gap-2">
            <input
              type="text"
              value={newFemaleChar}
              onChange={(e) => setNewFemaleChar(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), addFemaleCharacter())}
              className="flex-1 bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-pink-500"
              placeholder="Character name..."
            />
                ...
                <div key={i} className="flex items-center gap-2 bg-neutral-900 p-2 rounded-lg">
                  <label className="w-10 h-10 bg-neutral-700 rounded-lg flex items-center justify-center cursor-pointer overflow-hidden flex-shrink-0">
                    ...
                    <input 
                      ...
                      accept="image/*" 
                      className="hidden" 
                      onChange={(e) => handleFemaleCharImage(i, e.target.files?.[0] || null)}
                    />
                  </label>
                  <span className="flex-1 text-sm truncate">{char.name}</span>
        ...
        <button 
          ...
          className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3.5 rounded-lg font-bold text-lg disabled:opacity-50 transition-colors shadow-lg"
        >
